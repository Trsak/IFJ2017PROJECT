	...
	while (1) {
		...
		switch (state) {
			case 0: // The beginning
				...
				if (c == EOL) { // If there are multiple EOLs, returns only one and increments line counter with every EOL
					line++;
					c = getchar();
					...
					if (c == 34) { // ASCII 34 == "
						...
						printErrMsg(1, "Error on line: %d - The '!' was given and the following should be '\"' but %c was given", T.line, c);
						T.lexem = LEX_ERROR;
						return T;
					}
			...
			case 5: // It's a Number
				if (isdigit(c)) {
					...
					if (decimal || decimal_e) {
						printErrMsg(1, "Error on line: %d - Wrong format of a decimal number. Multiple dots were used.", T.line);
						T.lexem = LEX_ERROR;
						return T;
					}
					strAddChar(&T.value, c);
					...
					if (isdigit(c)) {
						...
						printErrMsg(1, "Error on line: %d - Wrong format of a decimal number. A number is required after a dot, but %c was given", T.line, c);
						T.lexem = LEX_ERROR;
						return T;
					}
					decimal = true;
					...
					if (decimal_e) {
						printErrMsg(1, "Error on line: %d - Wrong format of a decimal number. Multiple exponent expressions were used.", T.line);
						T.lexem = LEX_ERROR;
						return T;
					}
					decimal_e = true;
					...
					if (isdigit(c) || c == '+' || c == '-') {
						...
						printErrMsg(1, "Error on line: %d - Wrong format of a decimal number. A number or '+' or '-' is required after an exponent expression, but %c was given", T.line, c);
						T.lexem = LEX_ERROR;
						return T;
					}
				} else if (isalpha(c) || c == '_') {
					printErrMsg(1, "Error on line: %d - Wrong number format. %c char was given but a number is required", T.line, c);
					T.lexem = LEX_ERROR;
					return T;
				} else {
					ungetc(c, source);
			...
			case 6: // String
				if (c == EOL || c == EOF) {
					printErrMsg(1, "Error on line: %d - Wrong string format. '\"' is required to end the string but EOL or EOF was given", T.line);
					T.lexem = LEX_ERROR;
					return T;
				} else if (c == 34) {
